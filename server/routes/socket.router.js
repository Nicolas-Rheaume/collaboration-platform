...
const {connections, Connection} = require('../controllers/connection.controller');
...
module.exports = (io) => {
    io.on('connection', socket => {
        ...
        // EXAMPLE
        socket.on('example', async (index) => {
            try {   
                const response = await Example.example().catch(err => { throw err; });
                socket.to(socket.path).broadcast.emit("explorer/update", {});
            } catch(err) { 
                socket.emit("editor/error", err); 
            }
        });
